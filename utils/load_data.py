class Steinmetz():
	...
	def __init__(self,alldat):
  	...
  	def find_session(self,query_regions):
		...
		"""
		hits = {}
		for sess in range(len(alldat)):
			num_neurons = {}
			intersections = np.full(shape=alldat[sess]['brain_area'].shape,fill_value=True)
			for query_region in query_regions:
				intersections = np.intersect1d(intersections, alldat[sess]['brain_area']==query_region)
				num_neurons[query_region] = (alldat[sess]['brain_area']==query_region).sum()
